...
async def delete_member(company_id: int, member_id: int, db: Database = Depends(get_db), user: UserSchema = Depends(get_current_user)) -> ResponseMembershipSchema:
...
async def create_admin(company_id: int, payload: AddMembership, db: Database = Depends(get_db), user: UserSchema = Depends(get_current_user)) -> ResponseMembershipSchema:
...
async def downgrade_admin(company_id: int, user_id:int, db: Database = Depends(get_db), user: UserSchema = Depends(get_current_user)) -> ResponseMembershipSchema:
